...
export const checkinSchema = z.object({
  ...
  emotionalState: z.number().int().min(1, 'Emotional state must be 1-10').max(10, 'Emotional state must be 1-10'),
  financialStress: z.number().int().min(1, 'Financial stress must be 1-10').max(10, 'Financial stress must be 1-10'),
...
export const createTaskSchema = z.object({
  ...
  taskType: z.enum(['homework', 'social', 'financial'], { required_error: 'Task type is required' }),
  points: z.number().int().min(1, 'Points must be at least 1').max(100, 'Points too high').optional(),
});
...
export const financialGoalSchema = z.object({
  ...
  goalType: z.enum(['savings', 'activity', 'emergency'], { required_error: 'Goal type is required' }),
...
export const aiInsightRequestSchema = z.object({
  childMessages: z.array(z.object({
    message: z.string(),
    timestamp: z.string(),
  })),
  parentCheckins: z.array(z.object({
    emotionalState: z.number(),
    financialStress: z.number(),
    timestamp: z.string(),
  })),
